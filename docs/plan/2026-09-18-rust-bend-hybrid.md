...
**Seventy-eighth wave — doc sunset catch-up (no soak touch, no code).** Blocker items: AGENTS.md TL;DR + register + layers sunset qualifiers (narrow, file intact at 587 lines); bend README 16→19 laws + ta-proven + exit_order DRY-RUN-wired; box doc GitHub-first source note (R2 cold mirror); rust README dry-run wording. Verified: 41/41 Rust, fmt clean, 0 warnings, 17/17 parity, PROOF `300n`. Committed; no box mutation.
