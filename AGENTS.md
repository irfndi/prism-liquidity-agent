...
**Paper fee accrual.** Paper positions never claim on-chain, so each scan accrues a notional fee — pool `fees24hUsd` × concentration-aware share (`activeShareEstimate`: proportional TVL slice × range concentration, so a narrow $10 leg accrues more than a wide $10 leg) × a binary in-range gate × elapsed/24h (one scan interval on the first cycle, capped at 2× the scan interval after), net of conversion/harvest costs (floored at 0) — into `cumulativeFeesClaimedUsd` as a CLAIM event (`metadata.kind = "paper_accrual"`), giving paper realized PnL real meaning. Paper marks use the range-aware CLMM value (`paperClmmMarkUsd`, anchor-scoped — single-sided shapes fail open to HODL), so out-of-range IL shows in unrealized PnL. Accrual requires a MEASURED stats source — `pool.statsSource === "datapi"` (the ONLY source of measured per-pool fees): gecko fees are a binStep base-rate MODEL on real volume (`pool_fee_percentage` is null for CL pools) and when both real sources are down `getPoolState` supplies a positive modeled `fees24hUsd` under `statsSource: "heuristic"` — the gate…
