...
| `MIN_YIELD_EXIT_AGE_MS` | `14400000`                                                      | Economic-exit maturity gate (forensics-driven): the fee/IL < 0.5 and yield-regression EXITs do NOT fire until a position is this old — the paper forensics showed 33-minute median holds exiting at a loss on temporary IL that reversed, arming cooldowns that starved ENTERS. Capital-protection exits (W15, IL dominance, dust, TVL drop, trailing stop, lifecycles) stay age-free. |
| `MARKET_SCAN_MAX_NEGATIVE_DRIFT_BINS` | `-8`                                              | [drift-gate] momentum guard: a normal/market-lane ENTER is rejected when the pool's net active-bin drift sits below this floor (cascading price). Runner entries (dip-ladder fills ON dips) and launch-lane entries (young pools, no history) are exempt. |
