...
3. `strategy.computeMetrics` produces pure metrics (fee/IL, volume authenticity, bin utilization, TVL velocity vs the previous `pool_snapshots` row). Metrics whose inputs are unavailable are reported as explicit "unknown" (`volumeAuthenticityKnown` / `feeIlRatioKnown` / `binUtilizationKnown` on `PoolMetrics`); unknown metrics skip their pre-filter/EXIT gates with a warning and block ENTER (fail-closed), never fabricate 1.0. The known flags SPLIT by what each source actually measures: `volumeAuthenticityKnown` is true exactly when `isMeasuredStatsSource(statsSource)` (datapi OR geckoterminal — both publish REAL volume/TVL), but `feeIlRatioKnown` is **datapi-only**: fees are measured ONLY from the Data API's real per-pool fee data, while gecko fees are a binStep base-rate MODEL on real volume (`pool_fee_percentage` is null for every CL pool). Consequently every FEE-driven gate — the `fee/IL < 0.5` EXIT, the `[fee-il-gate]` hard ENTER floor, and paper fee accrual — **skips modeled gecko fees rather than acting on them**, and under `statsSource: "heuristic"` (fabricated volume AND fees) BOTH flags are false so neither volume nor fee gates act on fabricated numbers — a made-up-low ratio neither forces an exit nor blocks entry. The ×1.5 weighted-score ENTER gate still consumes the ratio under gecko and acts CONSERVATIVELY there: modeled fees UNDERRATE real fees, so the ratio is understated and that gate is harder to pass, never easier — a high-quality gecko pool can still ENTER via the volume candidate path (its volume IS known). `binUtilizationKnown` is source-independent (on-chain bins). The `[fee-il-gate]` ENTER skip under heuristic does not admit the pool: the volume candidate gate requires `volumeAuthenticityKnown`, so a heuristic pool still cannot enter (it is held, not rejected for a fabricated ratio).
