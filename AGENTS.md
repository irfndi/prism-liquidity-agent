...
| `TOKEN_FAILURE_BLOCK_MS` | `3600000`                                                     | G6 token-level circuit breaker: a genuine live EXIT failure on a pool blocks new ENTERs into ANY pool holding either of that pool's tokens for this window (metadata `token_block:<mint>`). Metadata read/write errors fail open. |
