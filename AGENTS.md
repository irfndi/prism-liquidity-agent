...
- **Per-entry estimate** (`estimateEntrySolLamports` in `engine/entry-sol-budget.ts`): the existing `MIN_SOL_FOR_ENTRY_LAMPORTS` (0.18 SOL) per-entry floor (gas + rent + ATA + priority fee) plus the SOL-funded portion of the position size (`× 1.01` slippage buffer; the whole entry in solFunded mode, else the pool's SOL leg half) plus the `SOL_ENTRY_TRANSACTION_BUFFER_LAMPORTS` wSOL-wrap buffer when the pool has a native-SOL leg. Over-estimating is safe — the gate only skips.
- **Skip, never fail:** an ENTER that does not fit the budget is SKIPPED with an audited `[wallet-reserve]` decision (riskResult `approved: false`) + warning memory. It is **not** counted as a pool failure and does **not** arm the entry-failure backoff (the pool re-qualifies next cycle) — capacity-limited skips can never trip the `execution_failures` pause. The gate runs in the in-slot ENTER tail (after risk approval, before `executeLive`) and in the idle-redeploy pass's live path.
- **Pause-breaker exemption:** as defense-in-depth, an ENTER failing with a funding-condition error (`isInsufficientTokenBalanceError`: insufficient token balance / `INSUFFICIENT_USDC_BALANCE` / `INSUFFICIENT_BALANCE_AFTER_SWAP` / insufficient SOL) is also treated as capacity-limited — it does not increment `consecutiveExecutionFailures` nor `cycle.poolsFailed` (the existing 30min-6h entry-failure backoff still arms). The `execution_failures` pause now fires only on genuine swap/execution errors.
- **Ordering:** pools are scanned in fee-APR rank order in market-scan mode (`marketScanPools` is inserted in `feeAprPct × liquidityFactor` rank at refresh), so the budget funds the highest-APR qualifiers first; watchlist pools (user-approved) scan before the market universe by design.
