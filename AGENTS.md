...
**Rotation.** When the portfolio is full (`MAX_OPEN_POSITIONS`) and a market runner is available, `MARKET_SCAN_ROTATION_ENABLED` exits the **lowest-APR held position** (confidence 1) when the runner's APR ≥ the worst held APR × `MARKET_SCAN_ROTATION_APR_MULT` (default 5) — a 5,000% runner vs a flat 25% major rotates; a marginal 30% pool does not. The exit is deterministic and the ENTER is consumed the same cycle (no exit-and-reenter in one pass); the freed slot admits the runner next cycle. Held positions whose APR is unmeasured (zero/unknown) are never rotation targets — fail-closed, a fabricated low APR must not sell a real position. Defaults are OFF (paper-first); the paper instance enables the lane.
...
| `MARKET_SCAN_ROTATION_APR_MULT` | `5`                                                             | Minimum candidate-APR / worst-held-APR multiple before a rotation EXIT fires.                                      |
