...
function makeOutcomes(count: number, opts?: { winnersHaveHighSignals?: boolean }): OutcomeRecord[] {
...
describe("computeSignalWeights", () => {
  ...
  it("clamps weights to the floor", () => {
    const current = makeWeights({ feeIlRatio: 0.5 });
    ...
    expect(result.feeIlRatio).toBeGreaterThanOrEqual(0.3);
  });

  it("clamps weights to the ceiling", () => {
    const current = makeWeights({ feeIlRatio: 2.0 });
    const outcomes = makeOutcomes(20, { winnersHaveHighSignals: true });
    const result = computeSignalWeights(outcomes, current, {
      minOutcomes: 5,
      weightFloor: 0.3,
    ...
    expect(result.feeIlRatio).toBeLessThanOrEqual(2.5);
  });
...
describe("weightedEntryScore", () => {
  it("returns higher scores for better metrics", () => {
    ...
    const goodMetrics = makeMetrics({ feeIlRatio: 2.0, volumeAuthenticity: 0.9, binUtilization: 0.8 });
    const badMetrics = makeMetrics({ feeIlRatio: 0.5, volumeAuthenticity: 0.3, binUtilization: 0.2 });
  ...
  it("applies feeIlRatio weight correctly", () => {
    const weights = makeWeights({ feeIlRatio: 2.0, volumeAuthenticity: 0, binUtilization: 0, tvlUsd: 0, tvlVelocity: 0 });
  ...
  it("applies volumeAuthenticity weight correctly", () => {
    const weights = makeWeights({ feeIlRatio: 0, volumeAuthenticity: 2.0, binUtilization: 0, tvlUsd: 0, tvlVelocity: 0 });
  ...
  it("applies binUtilization weight correctly", () => {
    const weights = makeWeights({ feeIlRatio: 0, volumeAuthenticity: 0, binUtilization: 2.0, tvlUsd: 0, tvlVelocity: 0 });
  ...
  it("caps TVL contribution at 1.0 for pools with TVL > $1M", () => {
    const weights = makeWeights({ feeIlRatio: 0, volumeAuthenticity: 0, binUtilization: 0, tvlUsd: 2.0, tvlVelocity: 0 });
  ...
  it("returns 0 for all-zero weights", () => {
    const weights = makeWeights({ feeIlRatio: 0, volumeAuthenticity: 0, binUtilization: 0, tvlUsd: 0, tvlVelocity: 0 });
    const metrics = makeMetrics();
    const score = weightedEntryScore(metrics, weights);
    expect(score).toBe(0);
  });
