...
/** Build the installed bundle-wrapper script for an install dir. */
function buildBundleWrapperContent(installDir: string): string {
...
/** Best-effort removal of a temp wrapper file left behind by a failed rewrite. */
function cleanupTempWrapper(temporary: string | undefined): void {
...
function rewriteBundleWrapper(wrapperBin: string, installDir: string): void {
  ...
  try {
    ...
    const content = buildBundleWrapperContent(installDir);
    ...
    cleanupTempWrapper(temporary);
  }
}

/** Cross-device fallback: copy the versioned dir into place instead of renaming. */
function copyVersionedInstallDir(installDir: string, canonicalDir: string): string {
  try {
...
/** Resolve a failed versioned-dir rename: raced migration or cross-device copy. */
function applyMigrationFallback(
  installDir: string,
  canonicalDir: string,
  ...
  if (code === "EXDEV") {
    return copyVersionedInstallDir(installDir, canonicalDir);
  }
  return null;
}
...
function migrateVersionedInstallDir(installDir: string): string {
  ...
  try {
    ...
    const fallback = applyMigrationFallback(installDir, canonicalDir, code);
    if (fallback !== null) {
      return fallback;
    }
...
/** Restore the pre-update install after a failed bundle apply; always throws. */
function rollbackBundleInstall(
  installDir: string,
  backupDir: string,
  ...
  throw new UpdateAbort(
    `${failedOperation} failed — rolled back to previous version. ` + `Error: ${failureDetail}`,
  );
}
...
async function updateFromBundle(
  ...
  try {
    ...
    const failureDetail = smokeErr instanceof Error ? smokeErr.message : String(smokeErr);
    rollbackBundleInstall(
      installDir,
      backupDir,
      previousInstallDir,
      wrapperBin,
