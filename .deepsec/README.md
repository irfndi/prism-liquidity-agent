...
Currently configured project: `prism-dlmm` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/prism-dlmm/SETUP.md` to fill in
   `data/prism-dlmm/INFO.md`.
...
`--project-id prism-dlmm` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/prism-dlmm/`.
