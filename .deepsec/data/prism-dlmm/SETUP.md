# Agent setup for `prism-dlmm`

This is a deepsec scanning workspace. Project `prism-dlmm` was just registered
(target: `..`). Setup is incomplete — `data/prism-dlmm/INFO.md`
...
2. **Fill in `data/prism-dlmm/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id prism-dlmm
pnpm deepsec process --project-id prism-dlmm
```
