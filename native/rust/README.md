...
`JUPITER_API_KEY` (optional — `x-api-key` on the primary price host; the
...
- `rpc::get_active_id` (live mode, per open pool per tick) → the host's own binHistory ring: `getAccountInfo` (base64) + a pure `LbPair` parse — 8-byte discriminator `[33,11,49,98,181,101,177,13]` + `i32` LE at offset **76** (repr(C)/bytemuck walk: StaticParameters 32 + VariableParameters 32 + seeds/pair_type → 8+32+32+1+2+1). Validated EXACT against the TS SDK's `DLMM.getActiveBin().binId` live (2463 == 2463); a captured account fixture pins it offline (`lbpair_8eyb.bin`). Wave 95: drift / recovery windows / vol windows / band containment / recovery drift-dist all derive from this chain-fed ring (TS's in-memory binHistory mechanism) — the persisted `pool_snapshots.active_bin_id` proxy and the stored position-row bin are both retired; cold start per process matches a restarted TS engine.
